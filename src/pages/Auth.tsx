            ...
            {[
              {
                ...
                desc: "Seus dados fiscais organizados em um só lugar.",
              },
              {
                icon: ShieldCheck,
                title: "Segurança de Dados",
                desc: "Acesso controlado e proteção total das suas informações.",
              },
            ].map((feature, i) => (
              <div
                key={i}
                className="flex gap-4 items-start p-4 rounded-2xl bg-card/50 border border-border/50 backdrop-blur-sm"
              >
            ...
            <CardContent className="p-0 sm:p-2">
              <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)} className="w-full">
                <TabsList className="grid w-full grid-cols-2 h-12 bg-muted/50 p-1 lg:bg-muted/80">
                  <TabsTrigger value="signin" className="rounded-md transition-all data-[state=active]:shadow-sm">
                    Entrar
                  </TabsTrigger>
                  <TabsTrigger value="signup" className="rounded-md transition-all data-[state=active]:shadow-sm">
                    Cadastrar
                  </TabsTrigger>
                </TabsList>
                ...
                <div className="p-4 lg:p-6 space-y-6">
                  <TabsContent value="signin" className="mt-0 space-y-6 animate-in fade-in-50 duration-500">
                    <form onSubmit={handleSignIn} className="space-y-5">
                      ...
                      <Button
                        type="submit"
                        className="w-full h-11 text-base font-semibold transition-all hover:translate-y-[-1px] active:translate-y-[1px]"
                        disabled={submitting}
                      >
                        {submitting ? (
